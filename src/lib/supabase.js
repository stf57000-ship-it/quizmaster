...
export async function saveProgress(userId, state) {
  ...
  await supabase.from("user_progress").upsert({
    ...
    total_answered: state.totalAnswered||0,
    best_score:     state.bestScore||0,
    exam_count:     state.examCount||0,
    best_exam_score:state.bestExamScore||0,
    has_perfect:    state.hasPerfect||false,
    earned_badges:  state.earnedBadges||[],
    concours_played:state.concoursPlayed||{},
    errors:         state.errors||{},
    history:        state.history||{},
    quiz_history:   state.quizHistory||[],
    concour_date:   state.concourDate||null,
...
export async function loadProgress(userId) {
  ...
  try {
    ...
    return {
      ...
      bestExamScore: data.best_exam_score||0,
      ...
      concourDate:   data.concour_date||null,
    };
  } catch { return null; }
}
