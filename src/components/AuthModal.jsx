    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,35,66,0.5)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
      ...
      <div className="fade-up card" style={{ width: "100%", maxWidth: 420, padding: 32 }}>
        ...
        {mode !== "forgot" && (
          <div style={{ display: "flex", gap: 8, marginBottom: 24, background: "rgba(10,35,66,0.04)", borderRadius: 12, padding: 4 }}>
            {["login", "register"].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(null); setSuccess(null); setShowPwd(false); }}
                style={{ flex: 1, padding: "8px", borderRadius: 9, border: "none", cursor: "pointer", background: mode === m ? "var(--surface)" : "transparent", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.85rem", color: mode === m ? "var(--text)" : "var(--muted)", transition: "all 0.2s" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {mode === "register" && (
            <input className="input" placeholder="Prénom" value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} />
          )}
          ...
          {mode !== "forgot" && (
            <div style={{ position: "relative" }}>
              <input className="input" placeholder="Mot de passe" type={showPwd ? "text" : "password"} value={password}
                onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()}
                style={{ paddingRight: 44 }} />
              <button onClick={() => setShowPwd(v => !v)}
                style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--muted)", fontSize: "1rem", padding: 0, lineHeight: 1 }}>
                {showPwd ? "🙈" : "👁️"}
              </button>
            </div>
          )}
        ...
        {mode === "login" && !success && (
          <button onClick={() => { setMode("forgot"); setError(null); }}
            style={{ marginTop: 12, width: "100%", background: "none", border: "none", cursor: "pointer", fontSize: "0.82rem", color: "var(--muted)" }}>
            Mot de passe oublié ?
          </button>
        )}
        {mode === "forgot" && (
          <button onClick={() => { setMode("login"); setError(null); setSuccess(null); }}
            style={{ marginTop: 12, width: "100%", background: "none", border: "none", cursor: "pointer", fontSize: "0.82rem", color: "var(--teal)" }}>
