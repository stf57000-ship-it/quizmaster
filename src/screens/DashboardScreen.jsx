// src/screens/DashboardScreen.jsx — avec bouton résiliation honnête
...
import { CancelScreen } from "./CancelScreen.jsx";
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{ color:"var(--muted)", fontSize:"0.88rem", marginBottom:16, display:"flex", alignItems:"center", gap:8 }}>
        {user
          ? <><span style={{ width:7, height:7, borderRadius:"50%", background:"var(--teal)", display:"inline-block" }}/> Synchronisé · {userName}{isPremium?" 👑":""}</>
      ...
      {cancelDone && (
        <div className="fade-in" style={{ background:"rgba(29,184,164,0.08)", border:"1px solid rgba(29,184,164,0.2)", borderRadius:12, padding:"12px 16px", marginBottom:16, fontSize:"0.85rem", color:"var(--teal)" }}>
          ✓ Résiliation prise en compte. Merci pour ta confiance.
        </div>
      )}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:18 }}>
        <div className="card" style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            <div style={{ fontFamily:"var(--font-display)", fontWeight:900, fontSize:"1.8rem", color:"var(--text)", lineHeight:1 }}>{globalAccuracy}%</div>
            <div style={{ fontSize:"0.78rem", color:"var(--muted)", marginTop:3 }}>Précision</div>
          </div>
        </div>
        <div className="card" style={{ display:"flex", alignItems:"center", gap:14 }}>
          <ProgressRing value={Math.min(streak, 30)} max={30} color="#FF9800" size={72}/>
          <div>
            <div style={{ fontFamily:"var(--font-display)", fontWeight:900, fontSize:"1.8rem", color:"var(--text)", lineHeight:1 }}>{streak}j</div>
            <div style={{ fontSize:"0.78rem", color:"var(--muted)", marginTop:3 }}>Streak</div>
          </div>
        </div>
      </div>

      <div className="card" style={{ marginBottom:14 }}>
        <div style={{ fontSize:"0.7rem", letterSpacing:2, color:"var(--muted)", fontWeight:700, textTransform:"uppercase", fontFamily:"var(--font-display)", marginBottom:14 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:14 }}>
        {[
          {label:"Quiz totaux",     value:appState.totalQuizzes||0,  icon:"🎯"},
          {label:"Examens blancs",  value:appState.examCount||0,     icon:"📋"},
          {label:"Meilleur score",  value:`${appState.bestScore||0}%`, icon:"🏆"}
        ].map(s => (
      ...
      {!user && (
        <div style={{ background:"rgba(29,184,164,0.08)", border:"1px solid rgba(29,184,164,0.2)", borderRadius:14, padding:"16px 20px", display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          <div>
      ...
      {/* Bouton résiliation — visible uniquement pour les Premium */}
      {isPremium && user && (
        <div style={{ marginTop:8, paddingTop:20, borderTop:"1px solid var(--border)" }}>
          <div style={{ fontFamily:"var(--font-display)", fontWeight:700, fontSize:"0.8rem", color:"var(--muted)", marginBottom:10, textTransform:"uppercase", letterSpacing:1 }}>Abonnement Premium</div>
          <button onClick={() => setShowCancel(true)}
            style={{ background:"none", border:"none", cursor:"pointer", fontSize:"0.82rem", color:"var(--muted)", fontFamily:"var(--font-body)", textDecoration:"underline", padding:0 }}>
