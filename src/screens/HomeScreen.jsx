// src/screens/HomeScreen.jsx — UX optimisée pour les 18-25 ans
...
export function HomeScreen({ appState, user, userName, isPremium, dailyRemaining, onStart, onShowAuth, onShowPricing, onUpdateState, onConcoursSelect }) {
  ...
  const [showAllConcours, setShowAllConcours] = useState(false);
  ...
  const grouped = Object.entries(CONCOURS_CATEGORIES).map(([catKey, cat]) => ({
    catKey, cat,
    items: Object.entries(CONCOURS).filter(([, c]) => c.category === catKey)
  }));
  ...
  return (
    <div className="fade-up">
      ...
      {/* Bandeau utilisateur — seulement si connecté */}
      {user && (
      ...
      {daysLeft !== null && daysLeft > 0 && (
        <div style={{ background:`${daysLeft<=7?"var(--orange)":daysLeft<=30?"#FF9800":"var(--teal)"}10`, border:`1px solid ${daysLeft<=7?"var(--orange)":daysLeft<=30?"#FF9800":"var(--teal)"}25`, borderRadius:14, padding:"12px 18px", display:"flex", gap:12, alignItems:"center", marginBottom:20 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"0.95rem", color:daysLeft<=7?"var(--orange)":"var(--teal)" }}>
              J-{daysLeft} avant ton concours {daysLeft<=7?"⚠️":"🎯"}
            </div>
            <div style={{ fontSize:"0.76rem", color:"var(--muted)" }}>
              {daysLeft<=7 ? "Dernière ligne droite — intensifie !" : daysLeft<=30 ? "Bonne cadence — continue !" : "Chaque jour compte !"}
            </div>
          </div>
        </div>
      )}

      {/* Stats — masquées si zéro */}
      {hasStats && (
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:6 }}>
          {ALL_MODES.map(m => (
            <button key={m.id} onClick={() => setMode(m.id)}
              ...
              {m.id === "express" && mode !== "express" && (
                <div style={{ position:"absolute", top:-6, right:-4, background:"#FF9800", color:"#fff", fontSize:"0.55rem", fontWeight:700, padding:"2px 5px", borderRadius:8, fontFamily:"var(--font-display)" }}>3'</div>
              )}
        ...
        {mode === "express" && (
          <div className="fade-in" style={{ marginTop:10, background:"rgba(255,152,0,0.08)", border:"1px solid rgba(255,152,0,0.2)", borderRadius:10, padding:"10px 14px", fontSize:"0.8rem", color:"#E65100" }}>
            ⚡ 5 questions · 15 secondes chacune · Parfait pour réviser en 3 minutes chrono
          </div>
        )}
      </div>

      {/* Concours — affichage condensé avec "voir plus" */}
      <div style={{ marginBottom:16 }}>
        <div className="section-label" style={{ marginBottom:12 }}>Concours</div>

        {/* Paramédicaux toujours visibles */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:9, marginBottom:10 }}>
          {Object.entries(CONCOURS)
            .filter(([, c]) => c.category === "paramédical")
            .map(([key, c]) => (
        ...
        {/* Autres catégories — masquées par défaut */}
        {showAllConcours && grouped.filter(g => g.catKey !== "paramédical").map(({ catKey, cat, items }) => (
          <div key={catKey} style={{ marginBottom:16 }} className="fade-in">
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
              <span style={{ fontSize:14 }}>{cat.icon}</span>
              <span className="section-label" style={{ fontSize:"0.62rem" }}>{cat.label}</span>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:9 }}>
              {items.map(([key, c]) => (
                <button key={key} onClick={() => handleSelectConcours(key)}
                  style={{ background:selectedConcours===key?c.colorLight:"var(--surface)", border:`2px solid ${selectedConcours===key?c.color:"var(--border)"}`, borderRadius:14, padding:"14px 12px", cursor:"pointer", transition:"all 0.2s", textAlign:"left", transform:selectedConcours===key?"translateY(-2px)":"none" }}>
                  <div style={{ fontSize:22, marginBottom:5 }}>{c.icon}</div>
                  <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"0.82rem", color:selectedConcours===key?c.color:"var(--text)", lineHeight:1.3 }}>{c.label}</div>
                </button>
              ))}
        ...
        <button onClick={() => setShowAllConcours(s => !s)}
          style={{ background:"none", border:"1px solid var(--border)", borderRadius:20, padding:"7px 16px", cursor:"pointer", fontSize:"0.78rem", color:"var(--muted)", fontFamily:"var(--font-display)", fontWeight:700, width:"100%", marginTop:4 }}>
          {showAllConcours ? "Masquer ▲" : "🛡️ Voir Sécurité, FP, Lycée Pro... ▼"}
        </button>
      </div>
      ...
      {selectedConcours && ["quiz","flashcards","express"].includes(mode) && (
        <div style={{ marginBottom:20 }} className="fade-in">
          <div className="section-label" style={{ marginBottom:10 }}>
            Thème <span style={{ color:"var(--muted)", fontWeight:400, textTransform:"none", letterSpacing:0, fontSize:"0.85rem" }}>(optionnel)</span>
          </div>
      ...
      {/* Bouton lancer */}
      <button id="start-btn" onClick={handleStart} disabled={!selectedConcours}
      ...
      {/* Message si pas de concours sélectionné */}
      {!selectedConcours && (
        <div style={{ textAlign:"center", marginTop:12, fontSize:"0.82rem", color:"var(--muted)" }}>
          ↑ Sélectionne un concours pour commencer
        </div>
      )}
