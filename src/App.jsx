...
export default function App() {
  ...
  const handleOnboardingComplete=(prefs)=>{ localStorage.setItem("cs_onboarding_done","1"); if(prefs.concours){update(s=>({...s,preferredConcours:prefs.concours,preferredNiveau:prefs.niveau}));setSelectedConcours(prefs.concours);} setScreen("home"); };
  ...
  const handleStart=async({mode,concours,difficulty,theme})=>{
    ...
    try{
      ...
      let data;
      if(mode==="errors"){const errs=(appState.errors||{})[concours]||[];if(!errs.length){setLoadError("Pas encore d'erreurs !");return;}data=await generateQuiz(concours,difficulty,null,true,errs);}
      else if(mode==="exam")data=await generateExamBlanc(concours);
      else data=await quizPreloader.get(concours,difficulty,theme);
      const questions=mode==="express"?data.questions.slice(0,5):data.questions;
      consumeQuizSlot();update(s=>updateStreak({...s,concoursPlayed:{...(s.concoursPlayed||{}),[concours]:true}}));
      setSessionData({concours,mode:"quiz",questions,isExam:mode==="exam",isExpress:mode==="express"});setScreen("quiz");
    }catch(e){if(e.code==="RATE_LIMIT_EXCEEDED")setShowPricing(true);else setLoadError(e.message||"Impossible de générer.");}
    finally{setLoading(false);}
  };

  const handleAnswer=({question,isCorrect,concours})=>{ if(!isCorrect)update(s=>addError(s,concours,question)); };
  const handleFinish=({score,total,answers,isExam,isExpress})=>{
    ...
    setSessionData(prev=>({...prev,score,total,answers,isExam,isExpress}));setScreen("result");
    setTimeout(()=>showFloatingPoints(score*5+(isExam?50:10)),500);
    if(shouldTriggerSurpriseReward()&&score/total>=0.6)setTimeout(()=>setSurpriseReward(getRandomSurpriseReward()),1200);
  };
